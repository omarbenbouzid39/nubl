...
function loadJSON(file, fallback) {
  ...
  } catch (e) { console.error(`⚠️  Error loading ${file}:`, e.message); }
  return fallback;
}

// Debounced save — avoids hammering disk on every message
const saveTimers = {};
function saveJSON(file, data, key = file) {
  clearTimeout(saveTimers[key]);
  saveTimers[key] = setTimeout(() => {
    try { fs.writeFileSync(file, JSON.stringify(data, null, 2), 'utf8'); }
    catch (e) { console.error(`⚠️  Error saving ${file}:`, e.message); }
  }, 800); // 800ms debounce
}

function saveUsersNow() { saveJSON(USERS_FILE, usersDB, 'users'); }
function saveRoomsNow()  { saveJSON(ROOMS_FILE, rooms,   'rooms'); }
...
// Rooms: load from disk, reset active users (they disconnected)
const _savedRooms = loadJSON(ROOMS_FILE, {});
const rooms = {};
Object.values(_savedRooms).forEach(r => {
  rooms[r.id] = {
    ...r,
    users: {},   // reset connected users on restart
    messages: r.messages || []
  };
  // Clean up non-persistent empty rooms that were saved
  if (!r.persistent) delete rooms[r.id];
});

console.log(`📂 Loaded ${Object.keys(usersDB).length} users, ${Object.keys(rooms).length} persistent rooms`);
...
const MAX_MSGS     = 500; // more messages since we persist them
...
io.on('connection', socket => {
  ...
  socket.on('join_room', ({ roomId, username, password }, cb) => {
    ...
    if (username && usersDB[username]) {
      if (!usersDB[username].myRooms.includes(roomId)) {
        usersDB[username].myRooms.push(roomId);
        saveUsersNow();
      }
    }
...
function pushRoomMsg(room, msg) {
  ...
  // Persist after every message (debounced 800ms)
  saveRoomsNow();
}
