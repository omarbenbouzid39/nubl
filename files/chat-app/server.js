...
// Users: { username → { password, verified, avatar, bio, myRooms:[roomId] } }
const usersDB = {
  admin: { password: 'admin123', verified: true, avatar: '', bio: 'مدير المنصة', myRooms: [] }
};
...
const MAX_MSGS     = 1000;   // حفظ آخر 1000 رسالة لكل غرفة

// ─── REST: Room messages (للمستخدمين الجدد عند refresh) ───
app.get('/api/rooms/:roomId/messages', (req, res) => {
  const room = rooms[req.params.roomId];
  if (!room) return res.status(404).json({ error: 'الغرفة غير موجودة', messages: [] });
  res.json({ messages: room.messages, roomName: room.name, persistent: room.persistent });
});
...
app.post('/api/register', (req, res) => {
  ...
  usersDB[username] = { password, verified: false, avatar: '', bio: '', myRooms: [] };
...
app.post('/api/login', (req, res) => {
  ...
  res.json({ ok: true, username, verified: !!user.verified, avatar: user.avatar || '', bio: user.bio || '', myRooms: user.myRooms });
});
...
app.get('/api/profile/:username', (req, res) => {
  ...
  res.json({ username: req.params.username, verified: !!user.verified, avatar: user.avatar || '', bio: user.bio || '' });
});

app.post('/api/profile/update', (req, res) => {
  const { username, avatar, bio } = req.body;
  if (!usersDB[username]) return res.status(401).json({ error: 'غير مصرح' });
  usersDB[username].avatar = (avatar || '').substring(0, 300);
  usersDB[username].bio    = (bio || '').substring(0, 200);
  res.json({ ok: true });
});
...
app.post('/api/admin/verify', (req, res) => {
  ...
  console.log(`${verified ? '✅' : '❌'} Verification for: ${username}`);
  res.json({ ok: true, username, verified: !!verified });
});

// ─── Admin: List Users ───
app.get('/api/admin/users', (req, res) => {
  const list = Object.entries(usersDB).map(([username, u]) => ({
    username, verified: !!u.verified, avatar: u.avatar || '', bio: u.bio || '',
    isOnline: Object.values(activeSockets).some(s => s.username === username)
  }));
  res.json({ users: list });
});
...
io.on('connection', socket => {
  ...
  // ─── Join Room ───
  socket.on('join_room', ({ roomId, username, roomName }, cb) => {
    let room = rooms[roomId];

    // غرفة غير موجودة — إذا أرسل العميل اسمها (re-join بعد refresh) نعيد إنشاءها
    if (!room && roomName) {
      rooms[roomId] = {
        id: roomId, name: roomName, persistent: true,
        users: {}, messages: [], createdAt: new Date().toISOString()
      };
      room = rooms[roomId];
      console.log(`🔄 Re-created room on rejoin: "${roomName}" (${roomId})`);
    }
    ...
    // إرسال كامل تاريخ الرسائل للمنضم الجديد
    ...
    io.emit('rooms_updated', getRoomsList());
    console.log(`👤 ${username} joined room "${room.name}" — history: ${room.messages.length} msgs`);
  ...
  // ─── طلب صريح لتحميل الرسائل (عند reconnect) ───
  socket.on('load_messages', ({ roomId }, cb) => {
    const room = rooms[roomId];
    if (!room) return cb?.({ messages: [] });
  ...
  socket.on('delete_message', ({ msgId, roomId: rid }) => {
    ...
    if (!msg) return;
    const isOwner = msg.socketId === socket.id || msg.username === socket.username;
