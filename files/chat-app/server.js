...
const avatarUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    ok ? cb(null, true) : cb(new Error('Image files only'));
  }
});
...
app.post('/api/avatar', avatarUpload.single('avatar'), async (req, res) => {
  ...
  if (!username || !usersDB[username]) return res.status(401).json({ error: 'غير مصرح' });
  usersDB[username].avatar = `/uploads/${req.file.filename}`;
  res.json({ ok: true, avatar: usersDB[username].avatar });
});
...
(async () => {
  ...
  usersDB['admin'] = { passwordHash: hash, myRooms: [], avatar: null, verified: true, joinedAt: new Date().toISOString() };
...
app.post('/api/register', async (req, res) => {
  ...
  usersDB[username] = { passwordHash, myRooms: [], avatar: null, verified: false, joinedAt: new Date().toISOString() };
  console.log('✅ Registered:', username);
  res.json({ ok: true, username, avatar: null, verified: false });
});
...
app.post('/api/login', async (req, res) => {
  ...
  if (user.passwordHash) {
    ...
    if (valid) { user.passwordHash = await bcrypt.hash(password, SALT_ROUNDS); delete user.password; }
  }
  if (!valid) return res.status(401).json({ error: 'اسم المستخدم أو كلمة المرور خاطئة' });
  res.json({ ok: true, username, myRooms: user.myRooms, avatar: user.avatar || null, verified: user.verified || false });
});
...
app.post('/api/my-rooms', (req, res) => {
  ...
  const list = ids.map(id => { const r = rooms[id]; if (!r) return null; return { id: r.id, name: r.name, persistent: r.persistent, userCount: Object.keys(r.users).length }; }).filter(Boolean);
  res.json({ rooms: list });
});
...
app.get('/api/profile/:username', (req, res) => {
  ...
  const myRoomsList = (user.myRooms || []).map(id => { const r = rooms[id]; if (!r) return null; return { id: r.id, name: r.name, persistent: r.persistent, userCount: Object.keys(r.users).length }; }).filter(Boolean);
  const isOnline = Object.values(activeSockets).some(s => s.username === username);
  res.json({ username, avatar: user.avatar || null, verified: user.verified || false, joinedAt: user.joinedAt || null, rooms: myRoomsList, isOnline });
});

// ─── Rooms API ───
app.get('/api/rooms', (req, res) => {
  const list = Object.values(rooms).map(r => ({ id: r.id, name: r.name, persistent: r.persistent, userCount: Object.keys(r.users).length, msgCount: r.messages.length, createdAt: r.createdAt }));
  res.json({ rooms: list });
});

// ─── Admin API ───
app.get('/api/admin/stats', (req, res) => {
  const roomList = Object.values(rooms).map(r => ({ id: r.id, name: r.name, persistent: r.persistent, users: Object.values(r.users), userCount: Object.keys(r.users).length, msgCount: r.messages.length, messages: r.messages.slice(-20), createdAt: r.createdAt }));
  res.json({
    totalRooms: roomList.length,
    totalUsers: Object.keys(activeSockets).length,
    registeredUsers: Object.keys(usersDB).length,
    rooms: roomList,
    users: Object.entries(usersDB).map(([uname, u]) => ({ username: uname, avatar: u.avatar || null, verified: u.verified || false, joinedAt: u.joinedAt || null, isOnline: Object.values(activeSockets).some(s => s.username === uname) }))
  });
});
...
app.delete('/api/admin/rooms/:roomId', (req, res) => {
  ...
  Object.keys(room.users).forEach(sid => { const s = io.sockets.sockets.get(sid); if (s) { s.leave(roomId); s.emit('room_deleted', { roomId }); } });
  delete rooms[roomId];
...
app.post('/api/admin/verify', (req, res) => {
  ...
  usersDB[username].verified = !!verified;
  Object.entries(activeSockets).forEach(([sid, info]) => {
    if (info.username === username) { const s = io.sockets.sockets.get(sid); if (s) s.emit('verification_update', { verified: !!verified }); }
  });
...
io.on('connection', socket => {
  ...
  socket.on('app_join', ({ username }) => {
    ...
    const user = usersDB[username];
    if (user) socket.emit('profile_data', { avatar: user.avatar || null, verified: user.verified || false });
  ...
  socket.on('join_room', ({ roomId, username }, cb) => {
    ...
    if (username && usersDB[username] && !usersDB[username].myRooms.includes(roomId)) usersDB[username].myRooms.push(roomId);
...
function leaveRoom(socket, roomId) {
  ...
  if (!room.persistent && Object.keys(room.users).length === 0) { delete rooms[roomId]; console.log('🗑️ Auto-deleted:', room.name); }
  io.emit('rooms_updated', getRoomsList());
}

function buildMsg(type, data, socket, roomId) {
  const user = usersDB[socket.username] || {};
  return { id: genId(), type, ...data, username: socket.username || 'Unknown', socketId: socket.id, roomId, timestamp: new Date().toISOString(), reactions: {}, avatar: user.avatar || null, verified: user.verified || false };
}
