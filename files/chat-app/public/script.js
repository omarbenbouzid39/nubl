...
let reactMsgId    = null;
let verifiedUsers = {};   // username → true/false (cache)
...
window.doLogin = async function() {
  ...
  try {
    ...
    localStorage.setItem('wasl_user', JSON.stringify({ username, verified: !!data.verified, avatar: data.avatar || '', bio: data.bio || '' }));
    myUsername = username;
    if (data.verified) verifiedUsers[username] = true;
...
function enterLobby(savedRoomIds) {
  ...
  // Load verified badge in lobby username display
  const saved = localStorage.getItem('wasl_user');
  if (saved) {
    try {
      const u = JSON.parse(saved);
      if (u.verified) verifiedUsers[myUsername] = true;
      if (u.avatar) lobbyAvatar.style.backgroundImage = `url('${u.avatar}')`;
    } catch(_) {}
  }
  updateLobbyVerifiedBadge();
  ...
  // Refresh verified cache from server
  loadVerifiedCache();
}
...
async function loadVerifiedCache() {
  try {
    ...
    const data = await res.json();
    (data.users || []).forEach(u => { verifiedUsers[u.username] = !!u.verified; });
    updateLobbyVerifiedBadge();
  } catch {}
}

function updateLobbyVerifiedBadge() {
  const badge = document.getElementById('lobby-verified-badge');
  if (badge) badge.style.display = verifiedUsers[myUsername] ? 'inline-flex' : 'none';
}

function verifiedBadgeHTML(username) {
  if (!verifiedUsers[username]) return '';
  return `<span class="verified-badge" title="موثق">✔</span>`;
}
...
window.enterRoom = function(roomId, roomName) {
  socket.emit('join_room', { roomId, username: myUsername, roomName }, (res) => {
    if (res?.error) {
    ...
    // ─── حفظ الغرفة الحالية لإعادة الانضمام بعد refresh ───
    sessionStorage.setItem('wasl_room', JSON.stringify({ roomId, roomName: currentRoomName }));
    ...
    // Clear messages — room_history event will fill them
    messagesArea.innerHTML = `<div class="system-msg" id="loading-history">⏳ جاري تحميل الرسائل...</div>`;
...
window.backToLobby = function() {
  ...
  currentRoomId = null;
  sessionStorage.removeItem('wasl_room');   // مسح الغرفة المحفوظة
  showScreen('lobby');
  fetchAllRooms();
  fetchMyRooms();
};
...
function initSocket() {
  ...
  socket.on('connect', () => {
    ...
    // ─── إعادة انضمام تلقائي بعد انقطاع الاتصال أو refresh ───
    if (currentRoomId && currentRoomName) {
      // كان المستخدم في غرفة قبل الانقطاع
      setTimeout(() => {
        socket.emit('join_room', { roomId: currentRoomId, username: myUsername, roomName: currentRoomName }, (res) => {
          if (res?.ok) {
            sidebarRoomName.textContent = currentRoomName;
            headerRoomName.textContent  = currentRoomName;
...
function updateUsersList(users) {
  ...
  users.forEach(username => {
    ...
    item.innerHTML = `<div class="user-avatar">${username.charAt(0).toUpperCase()}</div><div class="user-name">${escapeHtml(username)}${verifiedBadgeHTML(username)}</div>`;
...
function renderMessage(msg) {
  ...
  wrapper.innerHTML = `
    <div class="msg-meta">
      ${!isMe ? `<span class="msg-username">${escapeHtml(msg.username)}${verifiedBadgeHTML(msg.username)}</span>` : ''}
...
(function init() {
  ...
  if (saved) {
    try {
      const { username, verified } = JSON.parse(saved);
      if (username) {
        myUsername = username;
        if (verified) verifiedUsers[username] = true;
        initSocket();

        // ─── استعادة الغرفة السابقة بعد refresh ───
        const savedRoom = sessionStorage.getItem('wasl_room');
        if (savedRoom) {
          try {
            ...
            if (roomId) {
              ...
              // انتظر اتصال السوكت ثم انضم للغرفة مباشرة
              setTimeout(() => {
                ...
                roomIdDisplay.textContent   = roomId;
                roomStatus.textContent      = 'جاري الاتصال...';
                messagesArea.innerHTML = `<div class="system-msg" id="loading-history">⏳ جاري تحميل الرسائل...</div>`;
                showScreen('chat');
                // الانضمام سيحدث في حدث connect تلقائياً
              }, 100);
              return;
            }
          } catch(_) {}
