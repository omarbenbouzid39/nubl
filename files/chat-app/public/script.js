...
function initSocket() {
  ...
  socket.on('room_history', ({ roomId: histRoomId, messages }) => {
    // Only apply if this history belongs to our current room
    ...
    if (!messages || !messages.length) {
      messagesArea.innerHTML = `<div class="welcome-msg">
        ...
        <small>ابدأ الدردشة الآن</small>
      </div>`;
    } else {
      // Cache profile info from history
  ...
  if (saved) {
    try {
      ...
      if (username) {
        ...
        // After socket connects, check if user was in a room before refresh
        const roomSession = sessionStorage.getItem('wasl_room');

        if (roomSession) {
          // User was in a room — restore it directly
          try {
            const { roomId, roomName, password } = JSON.parse(roomSession);
            // Wait for socket to connect then re-join
            const tryRestore = setInterval(() => {
              if (socket && socket.connected && mySocketId) {
                clearInterval(tryRestore);
                console.log('[Restore] Re-joining room after refresh:', roomId);
                // Show chat screen while restoring
                showScreen('chat');
                messagesArea.innerHTML = `<div class="welcome-msg">
                  <div class="welcome-icon" style="font-size:32px">⏳</div>
                  <p>جاري استعادة المحادثة...</p>
                </div>`;
                enterRoom(roomId, roomName, password);
              }
            }, 100);
            // Timeout after 5s — fallback to lobby
            setTimeout(() => {
              clearInterval(tryRestore);
              if (!currentRoomId) {
                sessionStorage.removeItem('wasl_room');
