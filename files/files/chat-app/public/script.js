...
window.enterRoom = function(roomId, roomName) {
  // Set state BEFORE emitting — room_history arrives before callback
  currentRoomId   = roomId;
  currentRoomName = roomName;

  // Show chat screen with loading state immediately
  ...
  socket.emit('join_room', { roomId, username: myUsername }, (res) => {
    ...
    // Update with confirmed room name from server
...
function initSocket() {
  ...
  socket.on('room_history', ({ roomId, messages }) => {
    // Only apply if this history belongs to the current room
    if (roomId && roomId !== currentRoomId) return;

    messagesArea.innerHTML = '';
    if (!messages || !messages.length) {
      messagesArea.innerHTML = `<div class="welcome-msg"><div class="welcome-icon">🔗</div><p>${escapeHtml(currentRoomName || 'الغرفة')}</p><small>ابدأ الدردشة الآن</small></div>`;
      return;
    }
